      ...
      <div className="container">
        <div className="mb-8 flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
          <div className="text-center md:text-left">
            ...
            <h2 className="mt-3 font-heading text-3xl font-bold uppercase leading-tight text-navy-950 md:text-4xl">
        ...
        >
          {doctors.map((doctor) => (
            <SwiperSlide key={doctor.slug} className="!h-auto py-1">
              ...
              >
                <div className="relative min-h-[390px] bg-gradient-to-br from-surface-blue to-white">
                  <Image
                    src={doctor.image}
                    alt={doctor.name}
                    fill
                  ...
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-navy-950/64 via-transparent to-transparent opacity-80" />
                  <Link
                    href={`/doctors/${doctor.slug}`}
                    className="absolute bottom-5 left-5 inline-flex min-h-10 items-center justify-center gap-2 rounded-lg bg-white/86 px-4 text-sm font-black text-navy-950 shadow-card backdrop-blur transition hover:bg-cyan-500 hover:text-navy-950"
                  >
                    View Details
                    <ArrowRight className="h-4 w-4" />
                  </Link>
                ...
                <div className="bg-gradient-to-r from-teal-700 to-navy-900 px-5 py-5 text-center text-white">
                  ...
                  <p className="mt-1 text-xs font-bold uppercase tracking-[0.12em] text-white/64">{doctor.department}</p>
                </div>
